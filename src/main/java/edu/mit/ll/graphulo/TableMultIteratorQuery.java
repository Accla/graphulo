 ...
 */
public class TableMultIteratorQuery extends BranchIterator implements OptionDescriber {
  private static final Logger log = LogManager.getLogger(TableMultIteratorQuery.class);
//  public static final String PREFIX_REMOTE = "R";
...
//    optDesc.put("numEntriesCheckpoint", "(do not specify if C not present) number of entries until sending back a progress monitor");
...
//            Integer.parseInt(entry.getValue());
    ...
    return RemoteSourceIterator.validateOptionsStatic(optAT) &&
        (optC.isEmpty() //|| (optC.containsKey("tableName") && optC.size() == 1)
            || RemoteWriteIterator.validateOptionsStatic(optC));
  }
  ...
  public SortedKeyValueIterator<Key, Value> initBranchAfterIterator(final SortedKeyValueIterator<Key, Value> source, Map<String, String> options, IteratorEnvironment env) throws IOException {
    // parse options
//    {
//      String str = options.get("trace");
//      if (str != null)
//        Watch.enableTrace = Boolean.parseBoolean(str);
//    }
